...
void ClothParticle::update(float dt) {
    ...
    if(backToInitPos.length() > 100) {
        backToInitPos.normalize();
        vel -= backToInitPos * 10.0f;
    }
...
//-------------------------------------------------------
// Para utilizar en el contourAnimator
void ClothController::initContour(ofPolyline poly)
{
	particlesNum = poly.size();
	for(int i=0 ; i<particlesNum ; i++)
	{
        ofVec3f pos = ofVec3f( poly[i].x, poly[i].y, 0);
        
        float mass = 2;
        
        ClothParticle* p = new ClothParticle(pos,mass,0.99);
...
//---------------------------
void ClothController::initMeshContour() {
    // build mesh from particles
    particlesMesh.setMode(OF_PRIMITIVE_TRIANGLES);
    particlesMesh.clear();
	...
	// cierra el contorno
	particlesMesh.addVertex(particles[0]->pos);
...
//---------------------------
void ClothController::drawMeshContour() {
	particlesMesh.setMode(OF_PRIMITIVE_LINES);
	particlesMesh.drawWireframe();
}
